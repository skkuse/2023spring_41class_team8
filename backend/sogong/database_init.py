...
df.to_sql(name='sogongapp_CodingProblem', con = conn, if_exists='replace', dtype=dtype, index = False)
df2.to_sql(name="sogongapp_CodingTestcase", con = conn, if_exists='replace', dtype=dtype2, index=False)
df3.to_sql(name='sogongapp_ethicsproblem', con = conn, if_exists='replace', dtype=dtype3, index = False)
conn.commit()
conn.close()
print("Success")